...
def benchmark_xgb(y_tr, y_te, class_names, task_name, objective, n_classes=None):
    print(f"\n{'='*56}")
    ...
    params = dict(n_estimators=500, max_depth=6, learning_rate=0.1,
                  ...
                  objective=objective, random_state=42,
    ...
    print(f"n_estimators=500  max_depth=6  lr=0.1  fit_time={fit_time:.2f}s")
...
benchmark_xgb(y_tr_bin, y_te_bin, ["normal", "fault"],
              "binary (normal vs fault)", objective="binary:logistic")

benchmark_xgb(y_train, y_test, ["normal", "mass1", "mass2", "mass3"],
              "4-class", objective="multi:softmax", n_classes=4)
